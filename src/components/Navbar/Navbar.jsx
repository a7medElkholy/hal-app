...
const ActionButton = ({ text, className = "" }) => (
  <span
    dir="rtl"
    className={`flex hover:text-white text-black items-center justify-between px-4 py-2 border-2 bg-white hover:border-white rounded-full text-lg font-semibold transition-all duration-300 hover:bg-transparent hover:scale-101 ${className}`}
  >
    <span>{text}</span>
    <span className="bg-[#013366] text-white p-2 hover:scale-102 mr-4 rounded-full">
      <ArrowIcon />
    </span>
  </span>
);
...
export default function Navbar() {
  ...
  return (
    <div>
      ...
      >
        <div dir="rtl" className="container mx-auto p-5 main-text">
          <div className="flex items-center justify-between md:flex-row-reverse">
            ...
            {/* Desktop links (left side on desktop) */}
            <ul className="hidden md:flex md:flex-row items-center gap-10">
              ...
              <li>
                <Link to="/contact">
                  <span className="rounded-3xl p-2 inline-flex items-center">
                    <ActionButton text="تواصل معنا" />
                  </span>
                </Link>
              </li>
            </ul>
            {/* Logo (right on desktop due to md:flex-row-reverse) */}
          ...
          {/* Mobile menu (in-flow, expands nav height) */}
          <ul
            id="mobile-menu"
            className={`${isOpen ? "flex" : "hidden"} md:hidden flex-col gap-6 mt-4 bg-[var(--bg-main)] p-5 rounded-lg`}
          >
            <li>
            ...
            <li>
              <NavLink to="/works" onClick={() => setIsOpen(false)}>
                أعمالنا
              </NavLink>
            </li>
            <li>
              <NavLink to="/showIdea" onClick={() => setIsOpen(false)}>
                اعرض فكرتك
              </NavLink>
            </li>
            <li>
